...
def generate_expression(min_operators, max_operators, range_limit):
    ...
    if max_operators == 0:
        ...
        if min_operators > 0:
            ...
            left_operators = random.randint(max(0, min_operators - 1), max_operators - 1)
            right_operators = random.randint(max(0, min_operators - 1 - left_operators), max_operators - 1 - left_operators)
        else:
            # 可以选择生成数值或运算符
            if random.choice(['number', 'operator']) == 'number':
                return number_to_string(generate_number(range_limit))
            else:
                operator = generate_operator()
...
def canonical_form(expr_str):
    ...
    # Reconstruct the expression in a standardized postfix form
    return ' '.join(postfix)
...
def evaluate_expression(expr_str):
    """
    使用递归下降解析器解析和计算表达式，返回 Fraction 类型的结果。
    """
    tokens = tokenize(expr_str)
    tokens.append('EOF')  # 结束标记
    pos = 0
...
def grade(exercise_file, answer_file):
    ...
    for idx, (ex_line, ans_line) in enumerate(zip(exercises, answers), 1):
        try:
            ...
            correct_answer = evaluate_expression(ex_expr)
...
def main():
    ...
    主函数，解析命令行参数并执行相应的功能。
    """
    parser = argparse.ArgumentParser(description='生成和批改算术题目。')
